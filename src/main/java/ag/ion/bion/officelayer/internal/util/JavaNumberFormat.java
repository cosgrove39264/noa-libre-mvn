...
public class JavaNumberFormat implements IJavaNumberFormat {
  ...
  /**
   * The colors appear in different languages in the the format pattern. Therefore they are listed
   * here in english and german. We need a better solution - but for now it works.
   */
        ...
        try {
          ...
          if (counter == 2) {
            if (javaNumberFormatter.getJavaNumberFormatCondition() == null) {
              javaNumberFormatter.setJavaNumberFormatCondition(
                  new JavaNumberFormatCondition(JavaNumberFormatCondition.LOWER, 0.00));
  ...
  /**
   * Returns formated number for java code. The method returns null if no suitable format pattern
   * for the submitted value is available.
   *
  ...
  public IJavaFormattedNumber formatNumber(double value) throws UtilException {
    try {
      JavaNumberFormatter javaNumberFormatter = javaNumberFormatterRegistry
          .getJavaNumberFormatter(value);
      if (javaNumberFormatter != null) {
        return new JavaFormattedNumber(javaNumberFormatter.format(value),
            javaNumberFormatter.getTextColor());
      }
      return null;
    } catch (Exception exception) {
  ...
  public IJavaFormattedNumber formatNumber(String value) throws UtilException {
    ...
    try {
      JavaNumberFormatter javaNumberFormatter = javaNumberFormatterRegistry
          .getJavaNumberFormatter(doubleValue);
      if (javaNumberFormatter != null) {
        return new JavaFormattedNumber(javaNumberFormatter.format(doubleValue),
            javaNumberFormatter.getTextColor());
      }
      return null;
    } catch (Exception exception) {
   ...
   * @author Andreas Bröker
   */
  private void analyzeBracketContent(String bracketContent,
      JavaNumberFormatter javaNumberFormatter) {
    ...
    JavaNumberFormatCondition javaNumberFormatCondition = isJavaNumberFormatCondition(
        bracketContent);
  ...
  /**
   * Returns color on the basis of the submitted bracket content. Returns null if the bracket
   * content is not a color.
   *
   * @param bracketContent content of a bracket
  ...
  private Color isColor(String bracketContent) {
     ...
     */
    if (bracketContent.equalsIgnoreCase(COLOR_BLACK_EN) || bracketContent
        .equalsIgnoreCase(COLOR_BLACK_DE)) {
      return Color.BLACK;
    } else if (bracketContent.equalsIgnoreCase(COLOR_BLUE_EN) || bracketContent
        .equalsIgnoreCase(COLOR_BLUE_DE)) {
      return Color.BLUE;
    } else if (bracketContent.equalsIgnoreCase(COLOR_CYAN)) {
      return Color.CYAN;
    } else if (bracketContent.equalsIgnoreCase(COLOR_GREEN_EN) || bracketContent
        .equalsIgnoreCase(COLOR_GREEN_DE)) {
      return Color.GREEN;
    } else if (bracketContent.equalsIgnoreCase(COLOR_MAGENTA)) {
      return Color.MAGENTA;
    } else if (bracketContent.equalsIgnoreCase(COLOR_RED_EN) || bracketContent
        .equalsIgnoreCase(COLOR_RED_DE)) {
      return Color.RED;
    } else if (bracketContent.equalsIgnoreCase(COLOR_WHITE_EN) || bracketContent
        .equalsIgnoreCase(COLOR_WHITE_DE)) {
      return Color.WHITE;
    } else if (bracketContent.equalsIgnoreCase(COLOR_YELLOW_EN) || bracketContent
        .equalsIgnoreCase(COLOR_YELLOW_DE)) {
      return Color.YELLOW;
    } else {
      return null;
    }
  ...
  /**
   * Returns java number format condition. Returns null if the content of the bracket is not a
   * format condition.
   *
   * @param bracketContent content of a bracket
  ...
  private class JavaNumberFormatterRegistry {
    ...
    public JavaNumberFormatter getJavaNumberFormatter(double value) {
      ...
      for (int i = 0; i < arrayList.size(); i++) {
        ...
        JavaNumberFormatCondition javaNumberFormatCondition = javaNumberFormatter
