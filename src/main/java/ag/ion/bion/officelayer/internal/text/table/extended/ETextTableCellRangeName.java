...
public class ETextTableCellRangeName implements ITextTableCellRangeName {
   ...
   * @author Miriam Sutter
   */
  public ETextTableCellRangeName(ITextTableCellRange[] textTableCellRanges, int lastTable,
      int firstTable, int columnCount) throws IllegalArgumentException {
    ...
    if (firstTable != 0) {
      startRowIndex =
          textTableCellRanges[0].getRangeName().getRangeStartRowIndex() + (firstTable) * (
              ITextTable.MAX_CELLS_IN_TABLE / columnCount) - 1;
    }
    endColumnIndex = textTableCellRanges[textTableCellRanges.length - 1].getRangeName()
        .getRangeEndColumnIndex();
    if (lastTable != 0) {
      endRowIndex =
          textTableCellRanges[textTableCellRanges.length - 1].getRangeName().getRangeEndRowIndex()
              + (lastTable) * (ITextTable.MAX_CELLS_IN_TABLE / columnCount) - 1;
    } else {
      endRowIndex = textTableCellRanges[textTableCellRanges.length - 1].getRangeName()
          .getRangeEndRowIndex();
    }
    this.rangeName = TextTableCellNameHelper
        .getRangeName(startColumnIndex, startRowIndex, endColumnIndex, endRowIndex);
  }
  //----------------------------------------------------------------------------
