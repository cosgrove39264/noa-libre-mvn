...
public class ByteArrayXInputStreamAdapter extends ComponentBase implements XInputStream, XSeekable {
   ...
   * @date 27.10.2006
   */
  public ByteArrayXInputStreamAdapter(InputStream inputStream,
      IOfficeProgressMonitor officeProgressMonitor) throws IOException {
   ...
   * @author Andreas Bröker
   */
  public ByteArrayXInputStreamAdapter(IOfficeProgressMonitor officeProgressMonitor, byte[] bytes)
    ...
    if (officeProgressMonitor != null) {
      officeProgressMonitor
          .beginTask(Messages.getString("ByteArrayXInputStreamAdapter_monitor_task_name"),
              length); //$NON-NLS-1$
    }
  }
  //----------------------------------------------------------------------------
  ...
  public int readBytes(byte[][] buffer, int size)
      throws com.sun.star.io.BufferSizeExceededException, com.sun.star.io.IOException {
    try {
      ...
      if (officeProgressMonitor != null) {
        if (!showBuildInformation) {
          ...
          officeProgressMonitor.beginSubTask(Messages
              .getString("ByteArrayXInputStreamAdapter_monitor_sub_task",
                  new Object[]{new Integer(counter), new Integer(length)})); //$NON-NLS-1$
        } else {
          officeProgressMonitor.beginSubTask(Messages
              .getString("ByteArrayXInputStreamAdapter__monitor_integrating",
                  new Integer(buildCounter++))); //$NON-NLS-1$
        }
   ...
   * @author Andreas Bröker
   */
  public int readSomeBytes(byte[][] buffer, int size)
      throws com.sun.star.io.BufferSizeExceededException, com.sun.star.io.IOException {
  ...
  public void seek(long position)
      throws com.sun.star.lang.IllegalArgumentException, com.sun.star.io.IOException {
    if (bytes != null) {
      if (position < 0 || position > length) {
        throw new com.sun.star.lang.IllegalArgumentException(
            "Invalid seek position."); //$NON-NLS-1$
      } else {
        pos = (int) position;
      }
    } else {
