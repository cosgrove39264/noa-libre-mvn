...
public class TextTableCloneService extends AbstractCloneService {
  ...
  public TextTableCloneService(ITextTable table, XTextDocument textDocument) throws CloneException {
    ...
    this.serviceFactory = (XMultiServiceFactory) com.sun.star.uno.UnoRuntime
  ...
  /**
   * Clones the table to the given position and then returns a reference. If the position is null,
   * then the clone is placed at the end of the document.
   *
  ...
  /**
   * Clones the chosen table to the given position and then returns a reference This method also
   * enables to  adopts the content of the table (the default is to adopt, otherwise the paramter
   ...
   * @author Sebastian Rösgen
   */
  public IClonedObject cloneToPosition(IDestinationPosition range, boolean adoptContent,
      PropertyKeysContainer propertyKeysContainer) throws CloneException {
   ...
   * @param adoptContent inidicates if all content and properties should be cloned, too
   * @param createSpace indicates if a new paragraph should be constructed to create some space
   * between the old and the new textElement
  ...
  private IClonedObject clonePreprocessor(IDestinationPosition position, boolean adoptContent,
      boolean createSpace, boolean generateReturnValue, PropertyKeysContainer propertyKeysContainer)
    ...
    try {
      ...
      ITextTable newTable = textDocument.getTextTableService()
          .constructTextTable(tablePropertyStore.getRows(), tablePropertyStore.getColumns());
      ...
      if (propertyKeysToCopy != null) {
        ...
        ((IPropertyStore) tablePropertyStore).getProperties()
            .copyTo(propertyKeysToCopy, newTableProperties);
      }
      ...
      for (int rows = 0; rows < rowArrayLength; rows++) {
        ...
        for (int columns = 0; columns < columnArrayLength; columns++) {
          ...
          CloneDestinationPosition destinationCell = new CloneDestinationPosition(textTableCell,
              textTableCell.getClass());
  ...
  /**
   * Gets the properties form the table and the cells and then stores them in the apropriate objects
   * designed for them.
   *
