...
public class Frame implements IFrame {
   ...
   *
   * @return layout manager of the frame or null if a layout manager is not available
  ...
  /**
   * Disables the command dispatch related to the submitted command URL. In order to activate the
   * dispatches you need to call {@link IFrame#updateDispatches()}.
   *
  ...
  /**
   * Adds a dispatch delegate for the submitted command URL. In order to activate the dispatches you
   * need to call {@link IFrame#updateDispatches()}.
   *
  ...
  /**
   * Removes the dispatch delegate for the submitted command URL. In order to activate the
   * dispatches you need to call {@link IFrame#updateDispatches()}.
   *
   * @param commandURL command URL to be used
   * @author Andreas Bröker
   ...
   * @throws NOAException if a dispatch for the submitted command URL can not be provided
   * @author Andreas Bröker
   * @date 14.06.2006
   */
   ...
   * @date 15.08.2006
   */
  private class DispatchProviderInterceptor implements XDispatchProviderInterceptor,
      XInterceptorInfo {
    ...
    /**
     * Queries for a dispatch. Returns null if the dispatch can not be provided.
     *
     * @param targetFrameName name of the target frame
     * @param searchFlags search flags to be used
     * @return dispatch or null if the dispatch can not be provided
     * @author Andreas Bröker
     * @date 15.08.2006
     */
    ...
    public XDispatch[] queryDispatches(DispatchDescriptor[] dispatchDescriptors) {
      ...
      for (int i = 0, n = dispatchDescriptors.length; i < n; i++) {
        ...
        XDispatch dispatch = queryDispatch(dispatchDescriptor.FeatureURL,
            dispatchDescriptor.FrameName, dispatchDescriptor.SearchFlags);
        if (dispatch != null) {
          list.add(dispatch);
        }
      }
    ...
    public String[] getInterceptedURLs() {
      ...
      if (disabledCommandURLs != null) {
        commandURLs = (String[]) disabledCommandURLs
            .toArray(new String[disabledCommandURLs.size()]);
      }
      if (delegatesMap != null) {
        if (commandURLs == null) {
          ...
          String[] newURLs = (String[]) delegatesMap.keySet()
              .toArray(new String[delegatesMap.size()]);
