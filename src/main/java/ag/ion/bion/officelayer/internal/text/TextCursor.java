...
public class TextCursor implements ITextCursor {
   ...
   * @throws IllegalArgumentException if the submitted OpenOffice.org XTextCursor or XTextDocument
  ...
  public TextCursor(ITextDocument textDocument, XTextCursor xTextCursor) {
    if (xTextCursor == null) {
      throw new IllegalArgumentException(
          "The submitted OpenOffice.org XTextCursor interface is not valid.");
    }
   ...
   *
   * @param mark indicates if the space between the current position and the end is to be marked
   ...
   *
   * @param mark indicates if the space between the current position and the start is to be marked
   * @author Sebastian Rösgen
   */
   ...
   * @param mark indicates if the space between the current position and the new one is to be
   * marked
   * @author Sebastian Rösgen
   */
   ...
   * @param mark mark indicates if the space between the current position and the new one is to be
   * marked
   * @author Sebastian Rösgen
   */
  ...
  /**
   * Returns the page number of the text cursor start, returns -1 if page number could not be
   * determined.
   *
   * @return the page number of the text cursor start, returns -1 if page number could not be
   * determined
   * @author Markus Krüger
   */
  ...
  /**
   * Returns the page number of the text cursor end, returns -1 if page number could not be
   * determined.
   *
   * @return the page number of the text cursor end, returns -1 if page number could not be
   * determined
   * @author Markus Krüger
   */
  ...
  public void insertPageBreak() throws NOAException {
    try {
      ...
      if (xCell == null) {
        xTextCursor.getText()
            .insertControlCharacter(xTextCursor, ControlCharacter.PARAGRAPH_BREAK, false);
      }
  ...
  public boolean supportsWordCursor() {
    ...
    XWordCursor xWordCursor = (XWordCursor) UnoRuntime
        .queryInterface(XWordCursor.class, xTextCursor);
    if (xWordCursor != null) {
  ...
  public boolean supportsParagraphCursor() {
    ...
    XParagraphCursor xParagraphCursor = (XParagraphCursor) UnoRuntime
        .queryInterface(XParagraphCursor.class, xTextCursor);
