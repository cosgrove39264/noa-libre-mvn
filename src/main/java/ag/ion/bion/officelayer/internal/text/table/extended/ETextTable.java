...
//----------------------------------------------------------------------------
   ...
   * @author Miriam Sutter
   */
  public ETextTable(ITextDocument textDocument, ITextTable textTable)
  ...
  public void addRows(int count) throws TextException {
    ITextTableCellRange textTableCellRangeClone = textTableManagement.getLastTextTable()
        .getCellRange(0, textTableManagement.getLastTextTable().getRowCount() - 1, columnCount - 1,
   ...
   */
  private void addRowsProperty(int count, ITextTableCellRange textTableCellRangeClone)
  ...
  public void addRows(int index, int count) throws TextException {
    if (textTableManagement.getTextTables().length == 1
        && textTableManagement.getFirstTextTable().getRowCount() + count < maxRowsInTable) {
      textTableManagement.getFirstTextTable().addRow(index, count);
      return;
    }
    IETextTableCellRange textTableCellRange = getCellRange(0, index, columnCount - 1,
        getRowCount() - 1);
    ...
    try {
      ETextTableCellRange tableCellRange = (ETextTableCellRange) textTableCellRange
          .getCloneService().cloneToPosition(destinationPosition, null).getClonedObject();
      ...
      ITextTableCellRange textTableCellRangeClone = table
          .getCellRange(0, rowIndex, columnCount - 1, rowIndex);
      ...
      if (help < 0) {
        ...
        IETextTableCellRange cellRange = getCellRange(0, index, columnCount - 1,
            index + (rowCount - rowIndex));
        clearTableRange(cellRange);
  ...
  public IETextTableCell getCell(int rowIndex, int columnIndex) throws TextException {
    String cellName =
        TextTableCellNameHelper.getColumnCharacter(columnIndex) + TextTableCellNameHelper
            .getRowCounterValue(rowIndex);
  ...
  public IETextTableCell getCell(String cellName) throws TextException {
    ...
    ITextTableCell textTableCell = textTableManagement.getTextTable(tableNumber)
        .getCell(TextTableCellNameHelper.getColumnCharacter(cellName) + rowNumber);
   ...
   * @author Miriam Sutter
   */
  public IETextTableCellRange getCellRange(int firstColumnIndex, int firstRowIndex,
    ...
    if (firstTableNumber == lastTableNumber) {
      textTableCellRanges.add(textTableManagement.getTextTable(firstTableNumber)
          .getCellRange(firstColumnIndex, getRowIndexInTable(firstTableNumber, firstRowIndex),
              lastColumnIndex, getRowIndexInTable(firstTableNumber, lastRowIndex)));
    } else {
      ITextTable textTable = textTableManagement.getTextTable(firstTableNumber);
      textTableCellRanges.add(textTable
          .getCellRange(firstColumnIndex, getRowIndexInTable(firstTableNumber, firstRowIndex),
      ...
      for (int i = firstTableNumber + 1; i < lastTableNumber; i++) {
        ...
        textTableCellRanges.add(textTable
            .getCellRange(firstColumnIndex, 0, lastColumnIndex, textTable.getRowCount() - 1));
      }
      textTable = textTableManagement.getTextTable(lastTableNumber);
      textTableCellRanges.add(textTable.getCellRange(firstColumnIndex, 0, lastColumnIndex,
          getRowIndexInTable(lastTableNumber, lastRowIndex)));
    }
    ...
    ITextTableCellRangeName tableCellRangeName = new TextTableCellRangeName(firstColumnIndex,
        firstRowIndex, lastColumnIndex, lastRowIndex);
    IETextTableCellRange tableCellRange = new ETextTableCellRange(tableCellRanges,
        tableCellRangeName, this);
  ...
  public IETextTableRow[] getRows() throws TextException {
    ...
    for (int i = 0; i < textTables.length; i++) {
      ...
      try {
        textTableRows = (IETextTableRow[]) ArrayUtils
            .appendArray(textTableRows, rows, IETextTableRow.class);
      } catch (Exception exception) {
   ...
   * @author Miriam Sutter
   */
  private void cloneRange(ITextTable textTable, ITextTableCellRange textTableCellRange, int count,
  ...
  public void removeRows(int index, int count) throws TextException {
    ...
    if (help == 0) {
      textTableManagement.getTextTable(tableNumber)
          .removeRows(getRowIndexInTable(tableNumber, index), count);
    } else {
      int removed = textTableManagement.getTextTable(tableNumber).getRowCount() - 1;
      textTableManagement.getTextTable(tableNumber)
          .removeRows(getRowIndexInTable(tableNumber, index),
              textTableManagement.getTextTable(tableNumber).getRowCount() - 1);
      for (int i = 1; i < help; i++) {
        removed = removed + textTableManagement.getTextTable(tableNumber + 1).getRowCount();
        textTableManagement.getTextTable(tableNumber + 1)
            .removeRows(0, textTableManagement.getTextTable(tableNumber).getRowCount() - 1);
        ;
        textTableManagement.removeTextTable(tableNumber + 1, textDocument);
      }
      textTableManagement.getTextTable(tableNumber)
          .removeRows(getRowIndexInTable(tableNumber, index), count - removed);
    }
  }
  //----------------------------------------------------------------------------
  ...
  public void removeRows(int index) throws TextException {
    ...
    textTableManagement.getTextTable(tableNumber).removeRows(getRowIndexInTable(tableNumber, index),
        textTableManagement.getTextTable(tableNumber).getRowCount() - 1);
    for (int i = 1; i < help; i++) {
      textTableManagement.removeTextTable(tableNumber + 1, textDocument);
    }
