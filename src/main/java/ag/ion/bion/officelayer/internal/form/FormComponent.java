...
public class FormComponent implements IFormComponent {
   ...
   * @throws IllegalArgumentException if the submitted document or the OpenOffice.org XFormComponent
   * or XControlShape interface is not valid
   * @author Markus Krüger
   * @date 25.01.2007
   */
  public FormComponent(IDocument document, XControlShape xControlShape,
      XFormComponent xFormComponent) throws IllegalArgumentException {
    if (document == null) {
      throw new IllegalArgumentException(
          Messages.getString("Form.exception_document_invalid")); //$NON-NLS-1$
    }
    if (xFormComponent == null) {
      throw new IllegalArgumentException(
          Messages.getString("Form.exception_xformcomponent_interface_invalid")); //$NON-NLS-1$
    }
      ...
      if (xControlShape != null) {
        XControlAccess controlAccess =
            (XControlAccess) UnoRuntime.queryInterface(XControlAccess.class,
                ((ITextDocument) document).getXTextDocument().getCurrentController());
  ...
  /**
   * Returns the text range of the anchor where the form component starts, or null if not
   * available.
   *
  ...
  public ITextRange getStartTextRange() throws NOAException {
    XTextContent textContent = (XTextContent) UnoRuntime
        .queryInterface(XTextContent.class, xControlShape);
    if (textContent != null) {
