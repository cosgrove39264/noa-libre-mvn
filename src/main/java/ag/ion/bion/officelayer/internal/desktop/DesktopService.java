...
public class DesktopService implements IDesktopService {
   ...
   * @throws IllegalArgumentException if the submitted OpenOffice.org XDesktop interface or the
   * connection to OpenOffice.org is not valid
   * @author Andreas Bröker
   * @author Markus Krüger
   */
  public DesktopService(XDesktop xDesktop, IOfficeConnection officeConnection)
   ...
   */
  public DesktopService(XDesktop xDesktop, IOfficeConnection officeConnection,
      boolean preventTermination) throws IllegalArgumentException {
    if (xDesktop == null) {
      throw new IllegalArgumentException(
          "The submitted OpenOffice.org XDesktop interface is not valid.");
    }
    this.xDesktop = xDesktop;

    if (officeConnection == null) {
      throw new IllegalArgumentException(
          "The submitted connection to OpenOffice.org is not valid.");
    }
  ...
  public void addTerminateListener(ITerminateListener terminateListener) {
    ...
    TerminateListenerWrapper terminateListenerWrapper = new TerminateListenerWrapper(
        terminateListener,
  ...
  public void removeTerminateListener(ITerminateListener terminateListener) {
    ...
    TerminateListenerWrapper terminateListenerWrapper = (TerminateListenerWrapper) terminateListeners
        .get(terminateListener);
  ...
  /**
   * Adds new document listener. Uses GlobalEventBroadcaster to listen to any events on any
   * document.
   *
  ...
  public void addDocumentListener(IDocumentListener documentListener) throws DesktopException {
    try {
      ...
      if (eventBroadcaster == null) {
        Object globalEventBroadcaster = officeConnection.getXMultiServiceFactory()
            .createInstance("com.sun.star.frame.GlobalEventBroadcaster");
        eventBroadcaster = (XEventBroadcaster) UnoRuntime
            .queryInterface(XEventBroadcaster.class, globalEventBroadcaster);
      }
  ...
  /**
   * Constructs new OpenOffice.org frame which is integrated into the submitted AWT container. This
   * method works only on local OpenOffice.org applications.
   *
   * @param container AWT container to be used
   * @return new OpenOffice.org frame which is integrated into the submitted AWT container
  ...
  /**
   * Constructs new OpenOffice.org frame which is integrated into the submitted SWT container. This
   * method works only on local OpenOffice.org applications.
   *
  ...
  /**
   * Constructs new OpenOffice.org frame which uses the submitted native view. This method works
   * only on local OpenOffice.org applications.
   *
  ...
  public int getFramesCount() {
    ...
    XFrames xFrames = ((com.sun.star.frame.XFramesSupplier) UnoRuntime
        .queryInterface(com.sun.star.frame.XFramesSupplier.class, xDesktop)).getFrames();
    if (xFrames == null) {
      return -1;
    }
