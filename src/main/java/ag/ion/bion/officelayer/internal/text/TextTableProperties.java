...
public class TextTableProperties extends AbstractProperties implements ITextTableProperties {
   ...
   *
   * @param repeatHeadline information whether the first row is repeated on every page as headline
   ...
   *
   * @return information whether the first row is repeated on every page as headline
  ...
  public int[] getCellWidths() throws TextException {
    int widths[] = null;
    try {
      TableColumnSeparator[] columnSeparator = (TableColumnSeparator[]) getXPropertySet()
          .getPropertyValue("TableColumnSeparators");
      ...
      widths[columnSeparator.length] = (int) (width - columnSeparator[columnSeparator.length
          - 1].Position);
    } catch (Exception exception) {
      TextException textException = new TextException(exception.getMessage());
      textException.initCause(exception);
  ...
  public long getWidth() throws TextException {
    try {
      width = new Long(getXPropertySet().getPropertyValue("TableColumnRelativeSum").toString())
          .longValue();
      return width;
  ...
  public TextTableColumnsSeparator[] getTableColumnSeparators() throws TextException {
    try {
      TableColumnSeparator[] tableColumnSeparators = (TableColumnSeparator[]) getXPropertySet()
          .getPropertyValue("TableColumnSeparators");
