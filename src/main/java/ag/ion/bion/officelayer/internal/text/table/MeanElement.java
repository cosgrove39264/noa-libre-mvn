...
public class MeanElement implements ITextTableFormulaElement {
  ...
  public String replace() {
    if (formula.indexOf(TextTableFormula.MEAN) != -1) {
      MeanElement meanElement = new MeanElement(formula
          .substring(formula.indexOf(TextTableFormula.MEAN) + TextTableFormula.MEAN.length()));
      formula =
          formula.substring(0, formula.indexOf(TextTableFormula.MEAN)) + meanElement.replace();
    }
