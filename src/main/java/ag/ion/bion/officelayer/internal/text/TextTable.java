...
public class TextTable extends AbstractTextComponent implements ITextTable {
   ...
   * @throws java.lang.IllegalArgumentException if the submitted text document or OpenOffice.org
   * XTextTable interface is not valid
   * @author Andreas Bröker
   */
  ...
  /**
   * Returns the page number where the table starts, returns -1 if page number could not be
   * determined.
   *
   * @return the page number where the table starts, returns -1 if page number could not be
   * determined
   * @author Markus Krüger
   */
  ...
  /**
   * Sets the number of header rows to apply header style for. NOTE: Table must already be inserted
  ...
  /**
   * Returns the page number where the table ends, returns -1 if page number could not be
   * determined.
   *
   * @return the page number where the table ends, returns -1 if page number could not be determined
   * @author Markus Krüger
   */
  ...
  public void merge(ITextTableCellRange textTableCellRange) {
    ...
    String startCell =
        TextTableCellNameHelper.getColumnCharacter(cellRangeName.getRangeStartColumnIndex()) + (
            cellRangeName.getRangeStartRowIndex() + 1);
    String endCell =
        TextTableCellNameHelper.getColumnCharacter(cellRangeName.getRangeEndColumnIndex()) + (
            cellRangeName.getRangeEndRowIndex() + 1);
   ...
   * @param cellCount specifies the number of new cells that will be created for each cell contained
   * in the range.
   * @param vertical true if the range should be split vertically. Otherwise it will be split
   * horizontally.
   * @author Markus Krüger
   * @date 16.12.2009
   */
