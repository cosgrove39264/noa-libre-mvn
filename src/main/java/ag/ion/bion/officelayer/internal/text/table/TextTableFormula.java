...
public class TextTableFormula implements IFormula {
  ...
  public double calcFormula() throws TextException {
    try {
      ...
      return CalculateFormula
          .calculate(formulaElements, arguments, textTableFormulaExpression.getExpression());
  ...
  public ITextTableCellReferencesService getCellReferencesService() {
    if (textTableCellReferencesService == null) {
      textTableCellReferencesService = new TextTableCellReferencesService(
  ...
  private void findArguments(String formulaArgument) {
    ...
    if (posStart != -1 && posEnd != -1) {
      ...
      if (argumentBez.indexOf(":") != -1) {
        ...
        textTableFormulaExpression.setExpression(
            textTableFormulaExpression.getExpression().replaceFirst(argumentBez, helpString));
        ...
        textTableFormulaExpression.setExpression(
            textTableFormulaExpression.getExpression().substring(0, index) + "(" + pipe + ")"
                + textTableFormulaExpression.getExpression()
                .substring(index + argumentBez.length()));
      } else {
   ...
   * @author Miriam Sutter
   */
  private String getRangeArguments(String firstArguments, String secondArgument,
      Arguments arguments) {
  ...
  private void analyseBrackets(String formulaBracket, ArrayList bracketElements) {
    ...
    if (endPos != -1) {
      BracketElement bracketElement = new BracketElement(
          formulaBracket.substring(startPos, endPos + 1));
      ...
      bracketElement
          .setBracketElements((BracketElement[]) internalBracketElements.toArray(elements));
   ...
   * @author Miriam Sutter
   */
  private int getEndPos(String formulaBracket, int countBrackets, int i, char firstBracket,
      char lastBracket) {
  ...
  private void getMaxMin(boolean maximum, String formula, int formulaIndex) {
    ...
    if (index != -1) {
      ...
      for (int i = 0; i < help.length(); i++) {
        if (help.charAt(i) == '|') {
          helpIndex =
              maxMin.length() + (help.lastIndexOf("|") + help.substring(help.lastIndexOf("|"))
                  .indexOf(">")) + 1;
          found = true;
          String helpString = help.substring(oldIndex, i).trim();
          if (helpString.indexOf("<") != -1) {
            helpString = helpString.substring(helpString.indexOf("<"), helpString.indexOf(">") + 1)
                .trim();
            if (helpString.indexOf(":") != -1) {
            ...
            maxArguments.addArgument(argument);
            helpIndex =
                helpIndex + help.indexOf(new Double(argument.getValue()).toString()) + new Double(
                    argument.getValue()).toString().length();
          }
          oldIndex = i;
        }
      }
      ...
      if (helpString.indexOf("|") != -1) {
        ...
        if (helpString.indexOf("<") != -1) {
          helpString = helpString
              .substring(helpString.indexOf("|") + 1, helpString.indexOf(">") + 1);
          if (helpString.indexOf(":") != -1) {
            String[] helpArray = helpString.substring(1, helpString.length() - 1).split(":");
            getRangeArguments(helpArray[0], helpArray[1], maxArguments);
            Argument[] argumentsArray = (Argument[]) maxArguments.getArguments();
      ...
      textTableFormulaExpression.setExpression(
          textTableFormulaExpression.getExpression().substring(0, formulaIndex + index) + maxMin + (
              index + formulaIndex) + textTableFormulaExpression.getExpression()
              .substring(formulaIndex + index + helpIndex));
