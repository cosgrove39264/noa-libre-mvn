...
public class TextRange implements ITextRange {
   ...
   *
   * This method is not part of the public API. It should never used from outside.
   *
   ...
   *
   * @return cell of the texttable if the text range is part of cell or null if the text range is
   * not part of a cell
   * @author Andreas Bröker
   */
  ...
  /**
   * Compares this text range with the given text range. Returns 1 if this range starts before the
   * given text range, 0 if the text ranges start at the same position and -1 if this text range
   * starts behind the given text range.
   *
   * @param textRangeToCompare the text range to compare
   * @return 1 if this range starts before the given text range, 0 if the text ranges start at the
   * same position and -1 if this text range starts behind the given text range
    ...
    try {
      ...
      if (document instanceof ITextDocument) {
        ...
        if (thisCompareRangeCell != null && textRangeToCompareCell == null) {
          ...
          thisCompareRange = textDocument.getViewCursorService().getViewCursor()
          ...
          XTextViewCursorSupplier xTextViewCursorSupplier = (XTextViewCursorSupplier) UnoRuntime
              .queryInterface(XTextViewCursorSupplier.class,
                  textDocument.getXTextDocument().getCurrentController());
          xTextViewCursorSupplier.getViewCursor().goLeft((short) 1, false);
          textRangeToCompare = textDocument.getViewCursorService().getViewCursor()
              .getTextCursorFromEnd().getEnd();
        } else if (thisCompareRangeCell != null && textRangeToCompareCell != null) {
          XTextContent thisCompareRangeTable = thisCompareRangeCell.getTextTable()
              .getXTextContent();
          XTextContent textRangeToCompareTable = textRangeToCompareCell.getTextTable()
              .getXTextContent();
      ...
      try {
        return comparator.compareRegionStarts(thisCompareRange.getXTextRange().getStart(),
            textRangeToCompare.getXTextRange().getStart());
      } catch (Exception exception) {
  ...
  /**
   * Sets the document of the text range. This makes sense if the text range was generated as the
