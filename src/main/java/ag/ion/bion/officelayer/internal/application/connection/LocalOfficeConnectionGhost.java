...
/**
 * Office connection implementation. This class bases on the implementation of the OpenOffice.org
 * API Bean package.
 *
...
public class LocalOfficeConnectionGhost implements OfficeConnection {
   ...
   *
   * Sets up paths to the office application and native libraries if values are available in
   * <code>OFFICE_PROP_FILE</code> in the user home directory.<br /> "com.sun.star.beans.path" -
   * the
   * office application directory;<br/> "com.sun.star.beans.libpath" - native libraries directory.
   *
   ...
   *
   * Establishes a connection if necessary and initialises the UNO service manager if it has not
   * already been initialised. This method can return
  ...
  /**
   * Retrives the UNO component context. If no context is set, null will be returned. There will be
   * no try to connect.
   *
   * @return the office UNO component context
   * @author Markus Krüger
   ...
   *
   * The window is either a sub-class of java.awt.Canvas (local) or java.awt.Container (RVP).
   *
   ...
   * @throws MalformedURLException when inappropreate URL was provided
  ...
  private String getPipeName() {
    ...
    return replaceAll(
        replaceAll(java.net.URLEncoder.encode(aPipeName), "\\+", "%20"), "%",
  ...
  private class OfficeConnectionWrapper implements OfficeConnection {
     ...
     *
     * Establishes a connection if necessary and initialises the UNO service manager if it has not
     * already been initialised. This method can return <code>null</code> if it fails to connect to
  ...
  private class OfficeService {
    ...
    public void startupService() throws java.io.IOException {
      ...
      {
        cmdArray[4] =
            "-accept=pipe,name=" + getIdentifier() + ";" + protocol //$NON-NLS-1$ //$NON-NLS-2$
