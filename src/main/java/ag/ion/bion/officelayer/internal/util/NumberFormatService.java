...
public class NumberFormatService implements INumberFormatService {
   ...
   * @throws IllegalArgumentException if the OpenOffice.org XNumberFormatsSupplier interface is not
   * valid
   * @author Andreas Bröker
   */
  public NumberFormatService(ITextDocument textDocument,
      XNumberFormatsSupplier xNumberFormatsSupplier) throws IllegalArgumentException {
    if (xNumberFormatsSupplier == null) {
      throw new IllegalArgumentException(
          "Submitted OpenOffice.org XNumberFormatsSupplier interface is not valid.");
    }
  ...
  public INumberFormat getNumberFormat(int key) throws UtilException {
    ...
    try {
      ...
      XPropertySet docProps = (XPropertySet) UnoRuntime
          .queryInterface(XPropertySet.class, textDocument.getXTextDocument());
      Locale docLocale = (Locale) docProps.getPropertyValue("CharLocale");
      XNumberFormatTypes numberFormatTypes = (XNumberFormatTypes) UnoRuntime
          .queryInterface(XNumberFormatTypes.class, xNumberFormats);
   ...
   *
   * @param type the type to get all number formats for as constants of {@link
  ...
  public INumberFormat[] getNumberFormats(short type) throws UtilException {
    try {
      ...
      for (int i = 0; i < keys.length; i++) {
        ...
        if (((Short) xProp.getPropertyValue("Type"))
            .equals(new Short(com.sun.star.util.NumberFormat.CURRENCY)) &&
  ...
  /**
   * Converts a string which contains a formatted number into a number. If this is a text format,
   * the string will not be converted.
   *
