...
public class ETextTableCellRangeCloneService extends AbstractCloneService {
   ...
   * @author Miriam Sutter
   */
  public ETextTableCellRangeCloneService(IETextTableCellRange textTableCellRange,
  ...
  private IClonedObject clonePreprocessor(IDestinationPosition position, boolean adoptContent,
    ...
    try {
      ...
      range[0] = (ITextTableCellRange) textTableCellRanges[0].getCloneService()
          .cloneToPosition(position, adoptContent, propertyKeysContainer).getClonedObject();
      ...
      for (int i = 1; i < textTableCellRanges.length; i++) {
        ...
        range[i] = (ITextTableCellRange) textTableCellRanges[i].getCloneService()
            .cloneToPosition(destinationPosition, propertyKeysContainer).getClonedObject();
        table = range[i].getCell(0, 0).getTextTable();
        eTextTable.addTable(table);
      }
      if (generateReturnValue) {
        ITextTableCellRangeName textTableCellRangeName = new TextTableCellRangeName(0, 0,
            range[0].getColumnCount() - 1, range[0].getRowCount());
        IETextTableCellRange cellRange = new ETextTableCellRange(range, textTableCellRangeName,
            eTextTable);
