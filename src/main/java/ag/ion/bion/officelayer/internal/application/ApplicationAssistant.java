...
public class ApplicationAssistant implements IApplicationAssistant {
  ...
  private static final String KEY_MAIN_PART_OPEN_OFFICE = "OpenOffice"; //$NON-NLS-1$
   ...
   *
   * @throws OfficeApplicationException if the office application assitant can not be constructed
   * @author Andreas Bröker
   */
  ...
  /**
   * Returns informations about latest available local OpenOffice.org application, or null if none
   * was found.
   *
   * @return informations about latest available local OpenOffice.org application, or null if none
   * was found
    ...
    if (lazyApplicationInfos.length > 0) {
      ...
      for (int i = 0; i < lazyApplicationInfos.length; i++) {
        ...
        if (appInfo instanceof LazyOpenOfficeOrgApplicationInfo
            || appInfo instanceof LazyOpenOfficeApplicationInfo) {//for now, they are mostly compatible
  ...
  /**
   * Returns informations about latest available local LibreOffice application, or null if none was
   * found.
   *
   * @return informations about latest available local LibreOffice application, or null if none was
   * found
   * @author Markus Krüger
   * @date 04.03.2012
   */
  ...
  /**
   * Returns informations about latest available local office application, or null if none was
   * found.
   *
   * @return informations about latest available local office application, or null if none was found
   * @author Markus Krüger
   * @date 07.07.2010
   * @deprecated As now also LibreOffice is supported, this method is only keept for compatibility
   * reasons and returns the same as {@link #getLatestLocalOpenOfficeOrgApplication()}
   */
   ...
   * @date 07.07.2010
   * @deprecated As now also LibreOffice is supported, this method is only keept for compatibility
   * reasons and returns the same as null null null null     {@link #getLatestLocalOpenOfficeOrgApplication(IOfficeProgressMonitor)
   * )}
   */
  public ILazyApplicationInfo getLatestLocalApplication(
      IOfficeProgressMonitor officeProgressMonitor) {
    ...
    if (System.getProperty("oo.application.path") != null) {
      ...
      try {
        ...
        if (officeProgressMonitor != null) {
          officeProgressMonitor
              .beginTask(
                  Messages.getString("ApplicationAssistant.monitor_message_scannig_registry"),
                  possibleKeys.length); //$NON-NLS-1$
        }
          ...
          if (officeProgressMonitor != null) {
            officeProgressMonitor
                .beginSubTask(Messages
                    .getString(
                        "ApplicationAssistant.monitor_scanning_key",
                        possibleKeys[i])); //$NON-NLS-1$
          }
      ...
      try {
        if (officeProgressMonitor != null) {
          officeProgressMonitor
              .beginTask(
                  Messages.getString("ApplicationAssistant.monitor_looking_for_office_application"),
                  IOfficeProgressMonitor.WORK_UNKNOWN); //$NON-NLS-1$
        }
        ...
        if (officeProgressMonitor != null) {
          officeProgressMonitor
              .beginSubTask(Messages
                  .getString(
                      "ApplicationAssistant.monitor_buildung_application_infos")); //$NON-NLS-1$
        }
   ...
   *
   * Looks for application info on the basis of the submitted application home path. Returns null if
   * the application info can not be provided.
   *
   * @param home home path to be used
   * @return application info on the basis of the submitted application home path or null if the
   * application info can not be provided
   * @author Andreas Bröker
   */
    ...
    if (root.isDirectory()) {
      if (officeProgressMonitor != null) {
        officeProgressMonitor
            .beginSubTask(Messages
                .getString(
                    "ApplicationAssistant.monitor_scanning_directory",
      ...
      for (int i = 0, n = files.length; i < n; i++) {
        ...
        if (file != null) {
          ...
          if (!OSHelper.IS_WINDOWS) { // linux,unix,mac
            if (fileName.equals(APPLICATION_EXECUTEABLE + ".bin") || fileName
                .equals(APPLICATION_EXECUTEABLE + ".sh")) //$NON-NLS-1$ //$NON-NLS-2$
            {
  ...
  /**
   * Returns application properties on the basis of the submitted office home path. Returns null if
   * the application properties can not be found.
   *
   * @param home home of the office application
   * @return application properties on the basis of the submitted office home path or null if the
    ...
    for (majorVersion = 1; majorVersion < 10; majorVersion++) {
      for (minorVersion = 0; minorVersion < 10; minorVersion++) {
        for (updateVersion = 0; updateVersion < 10; updateVersion++) {
          arrayList.add("Applications\\" + keyMainPart + " " + majorVersion + "." + minorVersion
              + "\\shell\\edit\\command");
          arrayList.add(
              "Applications\\" + keyMainPart + " " + majorVersion + "." + minorVersion + "."
                  + updateVersion + "\\shell\\edit\\command");
          arrayList.add("Software\\" + keyMainPart + "\\" + keyMainPart + "\\" + majorVersion + "."
              + minorVersion);
          arrayList.add("Software\\" + keyMainPart + "\\" + keyMainPart + "\\" + majorVersion + "."
              + minorVersion + "." + updateVersion);
        }
      }
    }
