...
public class ETextTableCellRange implements IETextTableCellRange {
  ...
  public ETextTableCellRange(ITextTableCellRange[] textTableCellRanges,
      ITextTableCellRangeName textTableCellRangeName, IETextTable textTable)
    ...
    for (int j = 0; j < textTableCellRange.length; j++) {
      ...
      for (int i = 0; i < tableCells.length; i++) {
        textTableCells[i] = new ETextTableCell(tableCells[i],
            TextTableCellNameHelper.getColumnCharacter(tableCells[i].getName().getName())
                + rowNumber, textTable);
      }
      rowNumber++;
      try {
        this.textTableCells = (IETextTableCell[]) ArrayUtils
            .appendArray(this.textTableCells, textTableCells, IETextTableCell.class);
   ...
   * @author Miriam Sutter
   */
  public IETextTableCell getCell(int columnIndex, int rowIndex, int columnCount)
