...
public class TextContentService implements ITextContentService {
  ...
  public ITextDocumentTextShape constructNewTextShape(TextInfo textInfo) throws TextException {
    try {
      ...
      XPropertySet xProps = (XPropertySet) UnoRuntime
          .queryInterface(XPropertySet.class, xTextShape);
    ...
    try {
      if (textContent instanceof IParagraph) {
        ...
        XRelativeTextContentInsert xRelativeTextContentInsert = (XRelativeTextContentInsert) UnoRuntime
            .queryInterface(XRelativeTextContentInsert.class,
                xText);
  ...
  public void convertLinkedImagesToEmbeded() throws TextException {
    try {
      XTextGraphicObjectsSupplier graphicObjSupplier = (XTextGraphicObjectsSupplier) UnoRuntime
      ...
      for (int i = 0; i < names.length; i++) {
        ...
        if (xInfo.supportsService("com.sun.star.text.TextGraphicObject")) {
          ...
          if (graphicURL.indexOf("vnd.sun.") == -1) {
            XMultiServiceFactory multiServiceFactory = (XMultiServiceFactory) UnoRuntime
  ...
  /**
   * Returns the XTextContent interface of the paragraph just added, or null if not found.
   *
   * @param oldParagraphsBeforeInsert the old paragraphs array before the new paragraph is added
   * @return the XTextContent interface of the paragraph just added, or null if not found
    ...
    try {
      ...
      for (int i = 0; i < newParagraphs.length; i++) {
        ...
        XTextRangeCompare comparator = (XTextRangeCompare) UnoRuntime
            .queryInterface(XTextRangeCompare.class,
                text);
  ...
  private IParagraph[] getRealParagraphs() throws Exception {
    XEnumerationAccess xEnumerationAccess = (XEnumerationAccess) UnoRuntime
        .queryInterface(XEnumerationAccess.class,
