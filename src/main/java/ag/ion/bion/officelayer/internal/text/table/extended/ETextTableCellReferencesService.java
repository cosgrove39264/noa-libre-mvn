...
public class ETextTableCellReferencesService extends TextTableCellReferencesService {
  ...
  public ETextTableCellReferencesService(TextTableFormulaExpression textTableFormulaExpression,
      ETextTable textTable) throws IllegalArgumentException {
    ...
    for (int i = 0; i < textTableCellReferences.length; i++) {
      ...
      if (!startCell.getName().getName().equals(endCell.getName().getName())) {
        ...
        if (startTableIndex != endTableIndex) {
          formula =
              table.getName() + "." + startCell.getTableCell().getName().getName() + ":" + table
                  .getCell(endColumnIndex, table.getRowCount() - 1).getName().getName();
          ...
          for (int j = startTableIndex + 1; j < endTableIndex; j++) {
            ...
            formula = table.getName() + "." + TextTableCellNameHelper
                .getColumnCharacter(startCell.getName().getColumnIndex()) + "1" + ":" + table
                .getCell(endColumnIndex, table.getRowCount() - 1).getName().getName();
            textTableCellReference = new TextTableCellReference(formula);
            newTextTableCellReferences.add(textTableCellReference);
          }
          ...
          formula = table.getName() + "." + TextTableCellNameHelper
              .getColumnCharacter(startCell.getName().getColumnIndex()) + "1" + ":" + endCell
              .getTableCell().getName().getName();
        } else {
          formula =
              table.getName() + "." + startCell.getTableCell().getName().getName() + ":" + endCell
                  .getTableCell().getName().getName();
        }
      } else {
