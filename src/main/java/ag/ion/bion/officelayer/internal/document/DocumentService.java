...
public class DocumentService implements IDocumentService {
  ...
  public DocumentService(IOfficeConnection officeConnection, IServiceProvider serviceProvider)
    ...
    if (officeConnection == null) {
      throw new IllegalArgumentException(
          "The submitted office connection is not valid."); //$NON-NLS-1$
    }
  ...
  public static IDocument[] getCurrentDocuments(IServiceProvider serviceProvider)
    ...
    try {
      ...
      while (aktComponents.hasMoreElements()) {
        ...
        arrayList
            .add(DocumentLoader.getDocument((XComponent) a.getObject(), serviceProvider, null));
      }
  ...
  /**
   * Checks if the OpenOffice.org instance reached the number of maximum open documents and throws
   * an exception if this is the case.
   *
   * @param serviceProvider the serviceProvider to be used
   * @throws NOAException if the OpenOffice.org instance reached the number of maximum open
   * documents.
   * @author Markus Krüger
   * @date 11.11.2008
   */
    ...
    try {
      if (serviceProvider != null
        ...
        throw new NOAException(
            "The maximum number of opend documents was reached (Maximum number is "
                + IDocumentService.MAX_OPENED_DOCS //$NON-NLS-1$
                + ")."); //$NON-NLS-1$
      }
  ...
  public IDocument constructNewDocument(IFrame frame, String documentType,
    ...
    try {
      ...
      PropertyValue[] props = DocumentDescriptorTransformer
      ...
      if (frame != null) {
        ...
        xComponent = xComponentLoader
            .loadComponentFromURL(factoryURL, "_blank", 0, props); //$NON-NLS-1$
      }
   ...
   * @throws DocumentException if the document can not be loaded or the URL does not locate an
    ...
    try {
      ...
      if (document != null) {
        ...
        throw new DocumentException(
            Messages.getString("DocumentService_exception_url_invalid")); //$NON-NLS-1$
      }
    } catch (Throwable throwable) {
   ...
   * @param documentDescriptor document descriptor to be used
   * @return loaded document
   * @throws NOAException if the document can not be loaded or the URL does not locate an
   * OpenOffice.org document
   * @author Andreas Bröker
   * @date 02.07.2006
   */
  ...
  /**
   * Loads document on the basis of the submitted stream. May be <b>significantly slower</b> than
   * the load* variants which take an URL String as parameter.
   *
   ...
   * the store() method of the <code>IPersistenceService</code>. Furthermore OpenOffice.org can not
   * recognize if the document is already open - therefore the document will be never opened in
   * <code>ReadOnly</code> mode.</b>
   *
   * May be <b>significantly slower</b> than the load* variants which take an URL String as
   * parameter.
   *
  ...
  public IDocument loadDocument(IOfficeProgressMonitor officeProgressMonitor, IFrame frame,
    ...
    try {
      ...
      properties[0] = new PropertyValue("InputStream", -1, byteArrayToXInputStreamAdapter,
          PropertyState.DIRECT_VALUE); //$NON-NLS-1$
      properties = DocumentDescriptorTransformer.documentDescriptor2PropertyValues(properties,
          documentDescriptor);

      if (frame == null) {
        xComponent = xComponentLoader.loadComponentFromURL("private:stream", "_blank", 0,
            properties); //$NON-NLS-1$ //$NON-NLS-2$
      } else {
        xComponent = xComponentLoader
            .loadComponentFromURL("private:stream", frame.getXFrame().getName(),
                FrameSearchFlag.ALL, properties); //$NON-NLS-1$
      }
    ...
    if (officeProgressMonitor != null) {
      officeProgressMonitor
          .beginSubTask(Messages.getString("DocumentService_monitor_investigating")); //$NON-NLS-1$
    }
    IDocument document = DocumentLoader.getDocument(xComponent, serviceProvider, properties);
    if (document != null) {
      if (officeProgressMonitor != null) {
        officeProgressMonitor.beginSubTask(
            Messages.getString("DocumentService_monitor_loading_completed")); //$NON-NLS-1$
   ...
   * @return loaded document
   * @throws DocumentException if the document can not be loaded, the URL does not locate an
   * OpenOffice.org document of the submitted frame or URL is not valid
   * @author Andreas Bröker
   */
   ...
   * @return loaded document
   * @throws DocumentException if the document can not be loaded, the URL does not locate an
   * OpenOffice.org document or the submitted frame or URL is not valid
   * @author Andreas Bröker
   */
  ...
  private IDatabaseDocument constructDatabaseDocument() throws NOAException {
    try {
      Object dataSource = officeConnection.getXMultiComponentFactory()
          .createInstanceWithContext("com.sun.star.sdb.DataSource",
      ...
      XDocumentDataSource documentDataSource = (XDocumentDataSource) UnoRuntime
          .queryInterface(XDocumentDataSource.class,
              dataSource);
  ...
  private XComponentLoader constructComponentLoader() throws Exception {
    Object oDesktop = officeConnection.getXMultiComponentFactory()
        .createInstanceWithContext("com.sun.star.frame.Desktop",
            officeConnection.getXComponentContext()); //$NON-NLS-1$
