...
public class TextTableCellReferencesService implements ITextTableCellReferencesService {
  ...
  /**
   * Returns informations whether this formula has a reference to a text table cell with the
   * submitted name.
   *
  ...
  public void extendRowReferences(int rowIndex, int[] rowIndicesTo) throws TextException {
    ...
    for (int i = 0; i < textTableCellReferences.length; i++) {
      ...
      if (!textTableCellReference.isModified()) {
        ...
        if (textTableCellReference.containsRow(rowIndex) && !textTableCellReference
            .isRangeReference()) {
          ...
          for (int j = 0; j < rowIndicesTo.length; j++) {
            tmpTextTableCellReferences[j] = new TextTableCellReference(
                startColumnCharacter + (rowIndicesTo[j] + 1));
          }
          textTableFormulaModel
              .replaceCellReference(textTableCellReference, tmpTextTableCellReferences);
        }
      }
    }
  }
