...
public class TextTableCellCloneService extends AbstractCloneService {
  ...
  /**
   * Clones  the cell to the specified position, the boolean value indicates if the value is set to
   * true, the cell is appended to the specified position otherwise it is set before the position.
   *
  ...
  /**
   * Clones the chosen cell to the given position and then returns a reference This method also
   * enables to  adopts the content of the cell (the default is to adopt, otherwise the paramter has
   * to be set to false)
   *
   * @param adoptContent indicated if the content of the object should be adopted (this includes
   * formulas)
   * @param propertyKeysContainer container for property keys used for cloning style, my be null
  ...
  /**
   * Clones the chosen object to the given position. This method also  enables to  adopts the
   * content of the object (the default is to adopt, otherwise the paramter has to be set to false)
   *
   * @param position the positions the object is to be cloned to
  ...
  /**
   * Returns the stored value of the cell. This is done in the form of a Double object cause thus we
   * got to know if we actually had any kind of value in the array, and if we haven't we can return
   * null instead of any value.
   *
  ...
  /**
   * Alayses the whole cell, stores its formula and value in the clone service and creates a new
   * store object, which contains the important properties.
   *
   ...
   * @author Markus Krüger
   */
  private void getCellFromStore(TextTable table, boolean adoptContent,
      PropertyKeysContainer propertyKeysContainer) throws CloneException {
    try {
      if (textTableCellName == null) {
        textTableCellName = cellToClone.getName();
   ...
   * @param adoptContent indicates if the content should be adopted to the new cell (this includes
   * formulas)
   * @param generateReturnValue indicates weahter the return value will be generate or be null
   * @param propertyKeysContainer container for property keys used for cloning style, my be null
   ...
   * @author Markus Krüger
   */
  private IClonedObject recreateCell(ITextTableCell tableCell, boolean adoptContent,
      boolean generateReturnValue, PropertyKeysContainer propertyKeysContainer)
      throws CloneException {
    try {
      ...
      if (propertyKeysContainer != null) {
        propertyKeysToCopy = propertyKeysContainer
            .getPropertyKeys(ITextTableCellProperties.TYPE_ID);
      } else {
        //use default
      ...
      if (propertyKeysToCopy != null) {
        ...
        ((IPropertyStore) cellPropertyStore).getProperties()
            .copyTo(propertyKeysToCopy, newProperties);
      }
      ...
      if (propertyKeysToCopy != null) {
        ...
        ((IPropertyStore) firstCellCharacterPropertyStore).getProperties()
            .copyTo(propertyKeysToCopy, newProperties);
      }
  ...
  /**
   * Clones the chosen object to the given position and then returns a reference Between the given
   * position and the newly created object there will be a paragraph to add some space betwwen them.
   * So the object WILL NOT be merged together.
   *
   * This method is optional because it does not make sense to all possible implementors of the
   * interface. So it can happen that this method does nothing more or less than the cloneToPosition
   * method.
   *
  ...
  /**
   * Clones the chosen object to the given position. Between the given position and the newly
   * created object there will be a paragraph to add some space betwwen them. So the object WILL NOT
   * be merged together.
   *
   * This method is optional because it does not make sense to all possible implementors of the
   * interface. So it can happen that this method does nothing more or less than the cloneToPosition
  ...
  /**
   * Clones the chosen object after the given position and then returns a reference. Between the
   * given position and the newly created object there will be a paragraph to add some space betwwen
   * them. So the object WILL NOT be merged together.
   *
   * This method is optional because it does not make sense to all possible implementors of the
   * interface. So it can happen that this method does nothing more or less than the cloneToPosition
   * method.
   *
   * This method also  enables to  adopts the content of the object (the default is to adopt,
   * otherwise the paramter has to be set to false)
   *
   * @param position the destination position of the cloning
   * @param adoptContent indicated if the content of the object should be adopted
   * @param propertyKeysContainer container for property keys used for cloning style, my be null
  ...
  /**
   * Clones the chosen object after the given position. Between the given position and the newly
   * created object there will be a paragraph to add some space betwwen them. So the object WILL NOT
   * be merged together.
   *
   * This method is optional because it does not make sense to all possible implementors of the
