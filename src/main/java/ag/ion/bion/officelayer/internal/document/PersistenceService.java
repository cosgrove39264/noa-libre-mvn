...
public class PersistenceService implements IPersistenceService {
  ...
  private static final String ERROR_MESSAGE = Messages
      .getString("PersistenceService.general_error_message"); //$NON-NLS-1$
   ...
   * @throws IllegalArgumentException if the submitted document or the OpenOffice.org XStorable
  ...
  public PersistenceService(IDocument document, XStorable xStorable)
    ...
    if (document == null) {
      throw new IllegalArgumentException(
          Messages.getString("PersistenceService.exception_document_invalid")); //$NON-NLS-1$
    }
    this.document = document;

    if (xStorable == null) {
      throw new IllegalArgumentException(Messages
          .getString("PersistenceService.exception_xstorable_interface_invalid")); //$NON-NLS-1$
    }
   ...
   *
   * @return informations whether the doccument is in read only state or not
   * @author Andreas Bröker
   */
    ...
    try {
      ...
      if (throwable instanceof ErrorCodeIOException) {
        if (message == null || message.length() == 0) {
          message = Messages.getString("PersistenceService.error_io_message",
              String.valueOf(((ErrorCodeIOException) throwable).ErrCode)); //$NON-NLS-1$
        }
  ...
  public void store(String url) throws DocumentException {
    if (url == null) {
      throw new DocumentException(
          Messages.getString("PersistenceService.error_url_invalid_message")); //$NON-NLS-1$
    }
  ...
  public void export(String url, IFilter filter) throws DocumentException {
    ...
    if (filter == null) {
      throw new DocumentException(
          Messages.getString("PersistenceService.error_filter_invalid_message")); //$NON-NLS-1$
    }
  ...
  public void export(OutputStream outputStream, IFilter filter) throws NOAException {
    ...
    properties[0].Name = "FilterName"; //$NON-NLS-1$
    properties[0].Value = filterDefinition;
    properties[1] = new PropertyValue("OutputStream", -1, streamAdapter,
  ...
  private void storeInternal(OutputStream outputStream) throws Throwable {
    ...
    propertyValues[0] = new PropertyValue("OutputStream", -1, stream,
        PropertyState.DIRECT_VALUE); //$NON-NLS-1$
