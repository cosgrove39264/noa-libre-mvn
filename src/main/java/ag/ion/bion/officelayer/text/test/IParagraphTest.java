...
/**
 * JUnit test case for the IParagraph and the contained properties IParagprahPropertyStore and
...
public class IParagraphTest extends TestCase {
  ...
  public void testGetParagraphText() {
    try {
      ...
      IDocument document = localOfficeApplication.getDocumentService()
          .loadDocument(new FileInputStream(new File("test/testTexts.sxw")),
      ...
      if (document != null) {
        ...
        ITextContentEnumeration textEnumeration = textDocument.getTextService().getText()
            .getTextContentEnumeration();
        IParagraph[] paragraphs = textEnumeration.getParagraphs();

        ITextRange newTextRange = textDocument.getViewCursorService().getViewCursor()
            .getStartTextRange();
        XTextCursor cursor = newTextRange.getXTextRange().getText().createTextCursor();
        cursor.gotoEnd(false);
        newTextRange = new TextRange(textDocument, cursor);
        paragraphs[0].getCloneService()
            .cloneToPosition(new CloneDestinationPosition(newTextRange, newTextRange.getClass()),
                true, null);
