...
public class SimpleConnection {
   ...
   * @param port The port to connect to. A port value of zero (0) indicates a <b>local<b/>
   * connection
  ...
  public SimpleConnection(String connectionString, int port) throws Exception {
    ...
    } else if (connectionString != null && connectionString.length() > 1 && port > 0
        && port < 9999) {
      createServerConnection(connectionString, port);
    } else {
      throw new Exception(
          "Connection not possible with the given parameters: [" + connectionString + ":" + port
              + "]");
    }
  }
  ...
  public synchronized static SimpleConnection getConnection() {
    ...
    if (props.containsKey("OFFICE_HOST")) {
      if (Connection == null) {
        try {
          Connection = new SimpleConnection(props.getProperty("OFFICE_HOST"),
              Integer.valueOf(props.getProperty("OFFICE_PORT")));
  ...
  /**
   * Tries to start OO in headless server mode. <code>Give it at least 3-4 seconds before attempting
   * to use the server.</code>
   *
  ...
  public synchronized static void startOOServerIfNotRunning(final String path, final int port) {
    ...
    final String command =
        path.replace("\\", "\\\\") + File.separator + props.getProperty("OFFICE_BINARY_FOLDER")
    ...
    try {
      ...
      throw new UnsupportedOperationException(
          "Port " + port + " is already in use :-/. Not going to start OO here.");
   ...
   */
  private synchronized boolean createServerConnection(String host, int port)
      throws OfficeApplicationException, NOAException, InvalidArgumentException {
    if (host != null && port > 0) {
      ...
      try {
        ...
        try {
          ...
          Logger.getLogger(SimpleConnection.class.getName())
              .log(Level.SEVERE, null, officeApplicationException1);
        }
      }
      documentService =
      ...
      throw new InvalidArgumentException(
          "Host cannot be null and port must be > 0: " + host + ":" + port);
    }
   ...
   */
  private synchronized boolean createLocalConnection(String OOOPath)
      throws OfficeApplicationException, NOAException, InvalidArgumentException {
    if (OOOPath != null) {
      Map<String, String> configuration = new HashMap<String, String>();
