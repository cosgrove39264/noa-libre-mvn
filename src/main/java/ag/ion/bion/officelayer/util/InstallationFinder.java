 ...
 * com.sun.star.lib.loader.unopath system property or by setting the UNO_PATH environment variable
 * to the program directory of a UNO installation. Note, that Java 1.3.1 and Java 1.4 don't support
 * environment variables (System.getenv() throws java.lang.Error) and therefore setting the UNO_PATH
 * environment variable won't work with those Java versions. If no UNO installation is specified by
 * the user, the default installation on the system will be returned.</p>
 *
 ...
 * PATH environment variable. Note, that for Java 1.3.1 and Java 1.4 the default installation is
 * found by using the 'which' command, because environment variables are not supported with those
 * Java versions. Both methods require that the 'soffice' executable or a symbolic link is in one of
 * the directories listed in the PATH environment variable. For older versions than OOo 2.0 the
 * above described methods may fail. In this case the default installation is taken from the
 * .sversionrc file in the user's home directory. Note, that the .sversionrc file will be omitted
 * for OOo 2.0</p>
 */
...
final class InstallationFinder {
   ...
   *
   * @return the installation path or <code>null</code>, if no installation was specified or found,
   ...
   * The Java system property can be passed into the application by using the -D flag, e.g. java
   * -D<property name>=<installation path> -jar application.jar.</p>
   *
   * @return the installation path or <code>null</code>, if no installation was specified in the
   * Java system property or if an error occurred
   */
   ...
   *
   * <p>This method is called on all platforms.
   * Note, that in Java 1.3.1 and Java 1.4 System.getenv() throws java.lang.Error and therefore this
   * method returns null for those Java versions.</p>
   *
   * @return the installation path or <code>null</code>, if no installation was specified in the
   * environment variable or if an error occurred
   */
   ...
   *
   * @return the installation path or <code>null</code>, if no installation was found or if an error
   * occurred
   */
   ...
   * An installation is found, if the executable 'soffice' or a symbolic link is in one of the
   * directories listed in the PATH environment variable. Note, that in Java 1.3.1 and Java 1.4
   * System.getenv() throws java.lang.Error and therefore this method returns null for those Java
   * versions.</p>
   *
   * @return the installation path or <code>null</code>, if no installation was found or if an error
   * occurred
  ...
  /**
   * Gets the installation path from the 'which' command on Unix/Linux platforms.
   *
   * <p>This method is called on Unix/Linux platforms only.
   * An installation is found, if the executable 'soffice' or a symbolic link is in one of the
  ...
  /**
   * Gets the installation path from the .sverionrc file in the user's home directory.
   *
   * <p>This method is called on Unix/Linux platforms only.
   * The .sversionrc file is written during setup and will be omitted for OOo 2.0.</p>
   *
   * @return the installation path or <code>null</code>, if no installation was found or if an error
   * occurred
   */
  ...
  /**
   * Translates an OOo-internal absolute file URL reference (encoded using UTF-8) into a Java
   * canonical pathname.
   *
   * @param oooUrl any URL reference; any fragment part is ignored
   * @return if the given URL is a valid absolute, local (that is, the host part is empty or equal
   * to "localhost", ignoring case) file URL, it is converted into an absolute canonical pathname;
   * otherwise,
