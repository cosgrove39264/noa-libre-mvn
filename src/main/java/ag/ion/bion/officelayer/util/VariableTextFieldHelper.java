    ...
    try {
      ...
      XPropertySet xPropertySetField =
          (XPropertySet) UnoRuntime
              .queryInterface(XPropertySet.class, variableTextField.getXTextContent());
      ...
      if (numberFormat != null && numberFormat.getFormatType() == NumberFormat.FRACTION) {
        if (content.indexOf(' ') > -1) {
          ...
          if (content.indexOf('/') > -1) {
            ...
            content = "" + (Double.parseDouble(split[0]) + Double.parseDouble(split2[0]) / Double
                .parseDouble(split2[1]));
          } else {
      ...
      try {
        if (!isFormula && numberFormat != null
            && numberFormat.getFormatType() != NumberFormat.TEXT) {
      ...
      if (numberFormat != null && (numberFormat.getFormatType() == NumberFormat.DATE ||
        ...
        try {
          double doubelValue = numberFormatService
              .convertStringToNumber(numberFormat.getFormatKey(), content);
