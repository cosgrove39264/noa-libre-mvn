...
public class ScriptProvider implements IScriptProvider {
  ...
  public IScript[] getScripts() {
    XBrowseNode rootNode = (XBrowseNode) UnoRuntime
        .queryInterface(XBrowseNode.class, scriptProvider);
   ...
   *
   * @param list list to be used for the new builded scripts (can be null - a new list will be
   * builded)
  ...
  /**
   * Returns library node from the submitted type node. Returns null if a library with the submitted
   * name is not available.
   *
   * @param typeNode type node to be used
   * @param library name of the library
   * @return library node from the submitted type node or null if a library with the submitted name
   * is not available
   * @author Andreas Bröker
   * @date 13.06.2006
   */
